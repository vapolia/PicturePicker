		        ...
		        if (_picker.AutoSelectCameraImages && _picker.ShowCameraButton)
		        {
		            foreach (var collection in _collectionsFetchResultsAssets)
		            {
		                foreach (var fetchResult in collection)
		                {
		                    ...
		                    if (changeDetails?.InsertedObjects != null)
		                    {
		...
		private class GMAlbumsViewTableViewSource : UITableViewSource
		{
			...
			public override UITableViewCell GetCell (UITableView tableView, NSIndexPath indexPath)
			{
				var cell = (GMAlbumsViewCell)tableView.DequeueReusableCell (CellReuseIdentifier) 
				           ?? new GMAlbumsViewCell(UITableViewCellStyle.Subtitle, CellReuseIdentifier) {Accessory = UITableViewCellAccessory.DisclosureIndicator};
				...
				if (_parent._picker.DisplayAlbumsNumberOfAssets) {
					...
					cell.DetailTextLabel.Text = $"{assetsFetchResult.Count:0}";
			...
			public override async void RowSelected (UITableView tableView, NSIndexPath indexPath)
			{
			    ...
			    var gridViewController = new GMGridViewController(_parent._picker)
			    {
			        ...
			        AssetsFetchResults = _parent._collectionsFetchResultsAssets[indexPath.Section][indexPath.Row]
			    };
